			...
			if(jsonData.message!=null)
				{
					...
					row.innerHTML="<td>"+details[0]+"</td><td>"+details[1]+"</td><td>"+details[2]+"</td><td>"+details[3]+"</td><td><button class=\"btn btn-danger btn-sm\" onclick=\"sendInstructions('"+details[0]+"','ambulance','"+details[4]+"')\">Summon Ambulance</button><button type=\"button\" class=\"btn btn-primary btn-sm\" onclick=\"sendInstructions('"+details[0]+"','medication','"+details[4]+"')\">Suggest Medication</button></td><td>"+details[4]+"</td>";
				}
		}
		function sendInstructions(username,message,phone)
		{
			if(message=='medication')
			{
				...
				document.getElementById("submit_btn").addEventListener("click",function(){
					...
					var s1=username+","+message+","+medicine+","+description;
					console.log(s1);
					websocket.send(s1);
					medicine_name.value="";
			...
			{
				var s1=message+","+phone;
				console.log(s1);
				websocket.send(username+','+s1);	
			}
		}
	</script>
	<style>
